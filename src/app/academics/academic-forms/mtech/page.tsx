 ...
 */
function parseMTechData(html: string, files: { url: string; filename: string }[], domain: string) {
...
export default async function MTechFormsPage() {
    const DRUPAL_DOMAIN = getDrupalDomain();
    const INCLUDES = 'field_files';
    const data = await getDrupalData('/academics/academic-forms/mtech', INCLUDES);

    if (!data) notFound();

    const { forms, structures, syllabi } = parseMTechData(data.editor || "", data.files || [], DRUPAL_DOMAIN);

    const academicLinks = [
        { label: "B.Tech Forms", href: "/academics/academic-forms/btech" },
        { label: "M.Tech Forms", href: "/academics/academic-forms/mtech", active: true },
