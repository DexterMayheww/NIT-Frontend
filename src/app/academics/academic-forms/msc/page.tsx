 ...
 */
function parseMScData(html: string, files: { url: string; filename: string }[], domain: string) {
...
export default async function MScFormsPage() {
    const DRUPAL_DOMAIN = getDrupalDomain();
    const INCLUDES = 'field_files';
    const data = await getDrupalData('/academics/academic-forms/msc', INCLUDES);

    if (!data) notFound();

    const { forms, structures, syllabi } = parseMScData(data.editor || "", data.files || [], DRUPAL_DOMAIN);

    const academicLinks = [
        { label: "B.Tech Forms", href: "/academics/academic-forms/btech" },
        { label: "M.Tech Forms", href: "/academics/academic-forms/mtech" },
        { label: "M.Sc Forms", href: "/academics/academic-forms/msc", active: true },
