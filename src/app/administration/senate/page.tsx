// app/administration/senate/page.tsx
...
/**
 * Normalizes the Senate Members Table.
 * Ensures relative links point to Drupal domain and removes
 * redundant styling that might conflict with the institutional theme.
 */
function processSenateTable(html: string, domain: string) {
  if (!html) return '';
...
export default async function SenatePage() {
  ...
  const { data, status } = await getNodeByPath('/administration/senate');
  ...
  const quickLinks = [
    ...
    { label: 'Senate', href: '/administration/senate', active: true },
  ...
  const sanitizedHtml = processSenateTable(data.editor || '', DRUPAL_DOMAIN);
    ...
    <>
      <main className="min-h-screen bg-white pt-32 pb-20 px-4 font-sans">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-10">
          ...
          {/* --- MAIN CONTENT AREA: SENATE TABLE --- */}
          <div className="w-full lg:w-[65%] order-2 lg:order-1">
              ...
              <div className="overflow-x-auto">
                <article 
                  className="prose prose-lg max-w-none
                    ...
                    [&_thead_th]:bg-[#013a33] [&_thead_th]:text-white [&_thead_th]:p-4 [&_thead_th]:text-left [&_thead_th]:font-bold [&_thead_th]:border-none
            ...
            {!data.editor && (
              <div className="py-20 text-center bg-gray-50 border-2 border-dashed border-gray-200 rounded-xl">
                <p className="text-gray-400 italic">Senate membership details are currently being updated.</p>
              </div>
            )}
          </div>
