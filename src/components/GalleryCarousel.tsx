...
export function GalleryCarousel({ images }: { images: GalleryImage[] }) {
  ...
  const ITEM_WIDTH = 320; // Matches w-[320px]
  ...
  useEffect(() => {
    if (containerRef.current) {
      ...
      const itemCenterPoint = (activeIdx * (ITEM_WIDTH + GAP)) + (ITEM_WIDTH / 2);
  ...
  return (
    <div 
      ref={containerRef}
      className="relative w-full h-[700px] overflow-hidden flex items-center bg-transparent cursor-grab active:cursor-grabbing"
    >
      <div 
          ...
          return (
            <div
              ...
              className={`relative shrink-0 transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] will-change-transform
                w-[320px] h-[400px]
                ${isCenter ? 'z-30 scale-110 opacity-100' : 'z-10 opacity-40'}
